...
class MostPopularScreen extends StatefulWidget {
  const MostPopularScreen({super.key, required this.title, required this.sort});
  final String title;
  final String sort;
...
class MostPopularScreenState extends State<MostPopularScreen> {
  var currPage = '1';
  var currSort = 'popular-week';

  @override
  Widget build(BuildContext context) {
    currSort = widget.sort;
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            Flexible(
              child: ItemList(currPage: currPage, currSort: currSort, ),
            )
          ],
        ),
      ),
...
class ItemList extends StatefulWidget {
  const ItemList({super.key, required this.currPage, required this.currSort});
  final String currPage;
  final String currSort;
...
class ItemListState extends State<ItemList> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List>(
      future: ApiService.getSearchList(sort: widget.currSort, page: widget.currPage),
        ...
        if (snapshot.connectionState == ConnectionState.done) {
          if (snapshot.hasData) {
            return GridView.builder(
              ...
              itemBuilder: (_, index) {
                return ItemCard(dataId: snapshot.data![index][1], dataImage: snapshot.data![index][2], dataTitle: snapshot.data![index][0], dataIsDub: snapshot.data![index][3]);
              },
            );
          }
...
class ItemCard extends StatelessWidget {
  const ItemCard({super.key, required this.dataImage, required this.dataTitle, required this.dataId, required this.dataIsDub});
